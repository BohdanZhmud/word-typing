...
let private generateKey userId gameId userDisplayName = sprintf "%s:%s:%s" userId gameId userDisplayName

let setScore (userId: string) userDisplayName gameId rating = task {
    let entry = SortedSetEntry(RedisValue.op_Implicit(generateKey userId gameId userDisplayName), rating)
    ...
    return values
        |> Array.map (fun x ->
            ...
            |> Option.map (fun (userId, gameId, displayName) -> {
                score = { userId = userId; value = x.Score; gameId = gameId }; 
                userDisplayName = displayName }))
        |> Array.filter Option.isSome
        |> Array.map Option.get
}

let getUserScore (userId: string) userDisplayName gameId = task {
    let! score = db.SortedSetScoreAsync(sortedSetKey, RedisValue.op_Implicit(generateKey userId gameId userDisplayName))
